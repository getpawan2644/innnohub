...
<a href="{{ url('services/create') }}">ghfgj Add serviuces </a>
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th>@sortablelink('service_name', 'Service Name')</th>
									<th>Logo</th>
									<th>Description</th>
									<th>Created Date</th>
								...
								@if($records->count())
									...
									@foreach($records as $record)
										<tr>
											...
											<td><img src="{{asset('service/'.$record->logo) }}" style="width: 95px;"></td>
											<td>{{substr($record->description,0,50) }}</td>
											<td>@php $date=strtotime($record->created_at); @endphp {{ date('d-M-Y',$date)}}</td>
								
											<td align="center">
												<a href="{{URL('services/edit/'.$record->id)}}" class="action_btn"><i class="icofont-edit" style="font-size: 17px;"></i></a>
												<!--a href="{{URL('services/delete/'.$record->id)}}" class="action_btn"><i class="la la-pencil"></i> Delete</a-->
												<a href="javascript:void(0);" class="action_btn confirmDelete" data-action="{{ route('service.delete',$record->id) }}"><i class="icofont-delete" style="font-size: 17px;"></i></a>
