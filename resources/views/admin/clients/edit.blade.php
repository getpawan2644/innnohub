    ...
    <section class="page-content container-fluid">
        <div class="row">
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <form class="forms-sample" method="POST" action="{{ route('admin.clients.update',$record->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <select type="text" class="form-control" name="status" placeholder="Please choose a category">
                                            <option {{ old("status",$record->status) == \App\Models\Client::ACTIVE ? 'selected' : '' }} value="{{\App\Models\State::ACTIVE}}">{{\App\Models\State::ACTIVE}}</option>
                                            <option {{ old("status",$record->status) == \App\Models\Client::INACTIVE ? 'selected' : '' }} value="{{\App\Models\State::INACTIVE}}">{{\App\Models\State::INACTIVE}}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div class="form-group select-photo">
                                                <ul class="property-image-list">
                                                    @if(!empty(old('client_img',$record->client_img)))
                                                        ...
                                                        @endforeach
                                                    @else
                                                        @if(!empty($record['ClientImage']))
                                                            @foreach($record['ClientImage'] as $imgKey=>$imgValue)
                                                                <li class="mr-5 paidImage">
                                                                    <div class="coustom-input">
                                                                        <a href="javascript:void(0);" class="remove-logo-edit-pic"
                                                                           image_id="{{$imgValue['id']}}">X</a>
                                                                        <img
                                                                            src="{{asset('files/clientImg/'.$imgValue['client_id'].'/'.$imgValue['thumbnail'])}}"/>
                                                                        <input type="hidden" class="images_input"
                                                                               name="property[image_id][]"
                                                                               value="{{$imgValue['id']}}"/>
                                                                        <input type="hidden" class="images_input"
                                                                               name="property[images][]"
                                                                               value="{{$imgValue['image']}}"/>
                                                                        <input type="hidden" class="images_url"
                                                                               name="property[images_url][]"
                                                                               value="{{asset('files/clientImg/'.$imgValue['client_id'].'/'.$imgValue['image'])}}"/>
                                                                        <input type="hidden"
                                                                               class="thumbnail_images_input"
                                                                               name="property[thumbnail][]"
                                                                               value="{{$imgValue['thumbnail']}}"/>
                                                                        <input type="hidden" class="thumbnail_url"
                                                                               name="property[thumbnail_url][]"
                                                                               value="{{asset('files/clientImg/'.$imgValue['proclient_idperty_id'].'/'.$imgValue['thumbnail'])}}"/>
                                                                    </div>
                                                                    <div class="image-title upload">&nbsp;</div>
                                                                </li>
                                                            @endforeach
...
{{--                                            <div class="form-group">--}}
{{--                                                <label for="formGroupExampleInput">Youtube Video URL</label>--}}
{{--                                                <input type="text" id="video_url" class="form-control" name="video_url" value="{{ old('video_url', $record->video_url) }}">--}}
{{--                                                <iframe id="videoObject" type="text/html" width="500" height="265" class="youtube-viewer" style="display:{{!empty(old('youtube_url', $record->youtube_url))?:'none'}};" frameborder="0" allowfullscreen></iframe>--}}
{{--                                            </div>--}}
{{--                                        </div>--}}
                                    </div>
                            ...
                            <div class="card card-border-primary language_card">
                                <div class="card-header text-left">Videos</div>
                                <div class="card-body">
                                    ...
                                    <div class="appointment-div">
                                        @if(!empty(old('video_url')))
                                            ...
                                            @foreach(old('video_url') as $key => $value)
                                                <div class="row appointment">
                                                    <div class="col-lg-10 col-md-10 col-xs-10 col-sm-10">
                                                        ...
                                                        @if ($errors->has('video_url.'.$key))
                                                            <span class="error-message">
                                                                <strong>{{ $errors->first('video_url.'.$key) }}</strong>
                                                            </span>
                                                        @endif
                                                    </div>
                                                    @if($key!==0)
                                                        <div class="col-lg-2 col-md-2 col-xs-2 col-sm-2">
                                                            <div class="form-group align-middle" style="margin-top: 28px;">
                                            ...
                                            @endforeach
                                        @elseif(!empty($record->video_url_array))
                                            @php
                                                $i=0;
                                                $video_id = $record->video_id_array;
                                            @endphp
                                            @foreach($record->video_url_array as $key => $value)
                                                <div class="row appointment">
                                                    <div class="col-lg-10 col-md-10 col-xs-10 col-sm-10">
                                                        <div class="form-group from-time-dev">
                                                            <label for="formGroupExampleInput">Youtube Video URL</label>
